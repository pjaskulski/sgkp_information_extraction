...
# ------------------------------- MODELS ---------------------------------------
class MainLocalityModel(BaseModel):
    """ klasa reprezentująca oczekiwany model danych """
    chain_of_thought: List[str] = Field(
        ...,
        description="Kroki wyjaśniające prowadzące do ustalenia nazwy nadrzędnej (głównej) miejscowości której częścią jest opisywana w tekście miejscowość. Te kroki powinny dostarczać szczegółowego uzasadnienia wyboru nazwy miejscowości głównej."
    )
    locality: str = Field(None, description="Nazwa miejscowości nadrzędnej, której częścią jest opisywana w przekazanym tekście miejscowość. Nazwę miejscowości zwróć w mianowniku (lub pusty string jeżeli nie można ustalić)")
...
user_prompt = """
Przeanalizuj tekst dotyczący miejscowości opisanej w Słowniku Geograficznym Królestwa Polskiego
i wyszukaj w nim nazwę miejscowości nadrzędnej (głównej) do której należy opisywana
w tekście miejscowość.
"""
...
def get_data(tekst_hasla:str, client: OpenAI):
    ...
    completion = client.chat.completions.parse(
        ...
        response_format=MainLocalityModel,
        temperature=0
    )
...
def process_entry(entry_data: dict, client: OpenAI) -> dict:
    ...
    if rodzaj == "zbiorcze":
        ...
        for i, element in enumerate(elements):
            ...
            # czy hasło jest częścią miejscowości?
            is_part = False
            typ_punktu = element.get("typ_punktu_osadniczego", [])
            for item in typ_punktu:
                if item == "Część danej miejscowości (wsi, miasta)":
                    is_part = True
                    break
            if not is_part:
                continue

            # informacja o miejscowości nadrzędnej znajduje się zwykle na początku hasła
            if len(text) > 150:
                text = text[:150]
            ...
            try:
                ...
                if result.locality and value_test(result.locality):
                    element["miejscowosc_nadrzedna"] = result.locality
        ...
        # czy hasło jest częścią miejscowości?
        is_part = False
        typ_punktu = entry_data.get("typ_punktu_osadniczego", [])
        for item in typ_punktu:
            if item == "Część danej miejscowości (wsi, miasta)":
                is_part = True
                break
        ...
        if is_part:
            ...
            try:
                ...
                if result.locality and value_test(result.locality):
                    entry_data["miejscowosc_nadrzedna"] = result.locality
...
if __name__ == "__main__":
    ...
    data_path = Path('..') / 'SGKP' / 'JSON' / 'merged_dane_podstawowe' / f'sgkp_{VOLUME}_typy_punktow.json'
